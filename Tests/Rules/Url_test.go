...
func Test_it_can_validate_using_url_rule(t *testing.T) {
	...
	cases := []struct {
		...
		{[]byte(`{"Data": "http://www.google.dk:8000"}`), false},
		{[]byte(`{"Data": "https://127.0.0.1"}`), false},
		{[]byte(`{"Data": "http://127.0.0.1"}`), false},
...
func Test_it_can_validate_using_url_rule_with_localhost_param(t *testing.T) {
	// Setup
	cases := []struct {
		jsonString []byte
		shouldFail bool
		...
		{[]byte(`{"Data": "http://www.google.dk:8000"}`), false},
		{[]byte(`{"Data": "https://127.0.0.1"}`), false},
		{[]byte(`{"Data": "http://localhost"}`), false},
		{[]byte(`{"Data": "https://localhost"}`), false},
		{[]byte(`{"Data": "http://127.0.0.1"}`), false},
		{[]byte(`{"Data": "http://localhost:8000"}`), false},
		{[]byte(`{"Data": "https://localhost:8000"}`), false},
		{[]byte(`{"Data": "http://google"}`), true},
		{[]byte(`{"Data": "www.google.dk"}`), true},
		{[]byte(`{"Data": "google.dk"}`), true},
		{[]byte(`{"Data": "localhost"}`), true},
	...
	type testData struct {
		Data any `validation:"url:localhost"`
	}
