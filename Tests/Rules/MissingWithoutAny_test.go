...
func Test_it_can_validate_using_missingWithoutAny_rule(t *testing.T) {
	// Setup
	cases := []struct {
		jsonString []byte
		shouldFail bool
		...
		{[]byte(`{"Sibling1": true, "Data": true}`), true},
		{[]byte(`{"Sibling1": true, "Data": null}`), true},
		{[]byte(`{"Sibling1": null, "Data": null}`), true},
		{[]byte(`{"Sibling2": true, "Data": true}`), true},
		{[]byte(`{"Sibling2": true, "Data": null}`), true},
		{[]byte(`{"Sibling2": null, "Data": null}`), true},
		{[]byte(`{"Data": true}`), true},
	}

	type testData struct {
		Data     any `validation:"missingWithoutAny:Sibling1,Sibling2"`
		Sibling1 any
		Sibling2 any
	}
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			if testCase.shouldFail {
				...
				require.True(t, errorBag.HasFailedKeyAndRule("Data", "missingWithoutAny"))
