...
func Test_it_fails_require_one_in_group_if_no_fields_are_present_out_of_one(t *testing.T) {
	// Arrange
	var errorBag *JsonValidator.ErrorBag
	jsonString := []byte(`{}`)
	type testData struct {
...
func Test_it_accepts_require_one_in_group_if_exactly_one_field_out_if_one(t *testing.T) {
	// Arrange
	var errorBag *JsonValidator.ErrorBag
	jsonString := []byte(`{"Data1": true}`)
	type testData struct {
