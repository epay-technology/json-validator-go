...
func Test_it_can_validate_using_min_rule(t *testing.T) {
	// Setup
	cases := []struct {
		jsonString []byte
		shouldFail bool
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			if testCase.shouldFail {
				...
				require.True(t, errorBag.HasFailedKeyAndRule("Data", "min"))
