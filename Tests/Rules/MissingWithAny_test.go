...
func Test_it_can_validate_using_missingWithAny_rule(t *testing.T) {
	// Setup
	cases := []struct {
		jsonString []byte
		shouldFail bool
	...
	type testData struct {
		Data     any `validation:"missingWithAny:Sibling1,Sibling2"`
		Sibling1 any
		Sibling2 any
	}
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			if testCase.shouldFail {
				...
				require.True(t, errorBag.HasFailedKeyAndRule("Data", "missingWithAny"))
